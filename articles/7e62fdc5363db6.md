...
よって、**設定不備の脆弱性**を突いて内部のユーザ情報（`svc-alfresco`、`Service Accounts`、`test`）などを列挙出来た状況。
:::
...
:::message alert
LDAPの匿名バインド機能の無効化と、事前認証機能の無効化という**2つの脆弱性を掛け合わせることで**対象サーバへの侵入が成功する結果となった。
:::
...
:::message
最終的な権限昇格は、`Account Operators`によるアカウント作成権限と、`Exchange Windows Permissions`が持つドメインへの`WriteDacl`という**過剰な権限委譲（設定不備）**を連鎖させ、自作アカウントにDCSync権限を付与することで成立している。
:::

## まとめ

初期侵入から`SYSTEM`（≒ドメイン管理者相当）までの流れを振り返ると、以下の通り。
...
悪用したポイントを整理すると、以下の3つになる。

- **設定不備**: LDAPの匿名バインドが有効化されており、未認証でユーザ名を列挙できた
- **設定不備＋弱いパスワード**: サービスアカウントでKerberos事前認証が無効化されており、かつパスワードがrockyou.txtに載る強度だったため、AS-REP Roastingが成立した
- **過剰な権限委譲**: `Account Operators` → `Exchange Windows Permissions` → ドメインへの`WriteDacl`という権限の連鎖により、一般権限のアカウントからDCSync権限を自分で付与できた

いずれも単体では「よくある設定」に見えるが、これらが一直線に繋がることで一般ユーザからドメイン完全掌握まで到達してしまうのがこのマシンの怖いところ。

:::message
実務環境で同じ経路を塞ぐなら、ざっくり以下のような対策になる。

- LDAPの匿名バインドを無効化する（`dsHeuristics`の設定確認）
- サービスアカウントの「Kerberos事前認証を要求しない」設定を外す。パスワードは長いパスフレーズにし、可能ならgMSA（グループ管理サービスアカウント）へ移行する
- `Account Operators`をはじめとする組み込み特権グループのメンバーシップを棚卸しし、不要な付与を削除する。Exchange関連セキュリティグループのドメインACLも見直す
- DCSync（ディレクトリレプリケーション）の実行を監査ログで検知できるようにしておく

AD系のEasyマシンは「匿名列挙 → AS-REP Roasting → BloodHoundで権限昇格パス発見 → ACL悪用」というテンプレの宝庫なので、Forestはその一連の流れを最初に通しで体験する教材としてかなり良い。
:::
